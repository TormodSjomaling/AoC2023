...
var exampleEngineSchematic = "467..114..\n" +
                             "...*......\n" +
                             "...35..633\n" +
                             "......#...\n" +
                             "617*......\n" +
...
var symbols = FindAllSymbols(engineSchematic);

List<string> FindAllSymbols(string s)
{
    var symbols = new List<string>();
    foreach (var c in s)
    {
        if(!char.IsLetterOrDigit(c) && !Equals(c.ToString(), ".") && !Equals(c.ToString(), "\n"))
        {
...
{
    var sum = 0;
    var rows = engineSchematic.Split("\n", StringSplitOptions.RemoveEmptyEntries);

    for (var i = 0; i < rows.Length; i++)
    {
        const string pattern = @"\d+";
        var matches = Regex.Matches(rows[i], pattern);
        ...
        foreach (Match match in matches)
        {
            ...
            bool symbolBefore = startIndex > 0 && symbols.Any(s => s.Contains(rows[i][startIndex - 1]));
            bool symbolAfter = endIndex < rows[i].Length - 1 && symbols.Any(s => s.Contains(rows[i][endIndex + 1]));

            if (startIndex == 0 && symbolAfter)
            {
                sum += int.Parse(match.Value);
                continue;
            }

            if (endIndex == rows[i].Length - 1 && symbolBefore)
            {
                sum += int.Parse(match.Value);
                continue;
            ...
            if (i == 0)
            {
                ...
                {
                    if (symbols.Contains(rows[i + 1][j].ToString()))
                    {
                        sum += int.Parse(match.Value);
                    }
                }
            ...
            else
            {
                for (var j = from; j <= to; j++)
                {
                    if (symbols.Contains(rows[i - 1][j].ToString()) || symbols.Contains(rows[i + 1][j].ToString()))
                    {
                        sum += int.Parse(match.Value);
                    }
                }
...
var symbols2 = new List<string>() { "*" };

const string faenskap =
    "...............776..............552........968..................589...26...........484..............958......186....546.........484.........\n" +
    ".........*.........778....................*....124...................................*...............*........%..26.........................\n" +
    "......194..380....@....900..........639....467........478*..............582...........798.............326...........894.........#...........";
...
int PartTwo()
{ 
    var rows = faenskap.Split("\n", StringSplitOptions.RemoveEmptyEntries);
    var gearOne = 0;
    var gearTwo = 0;
    var gear = 0;
    var foundGearOneOnRow = 0;
    var foundGearTwoOnRow = 0;
    var gearOneIndex = 0;
    var gearTwoIndex = 0;
    ...
    for (var i = 0; i < rows.Length; i++)
    {
        ...
        foreach (Match match in matches)
        {
            ...
            bool symbolBefore = startIndex > 0 && symbols2.Any(s => s.Contains(rows[i][startIndex - 1]));
            bool symbolAfter = endIndex < rows[i].Length - 1 && symbols2.Any(s => s.Contains(rows[i][endIndex + 1]));

            if (startIndex == 0 && symbolAfter)
            {
                if (gearOne == 0)
                {
                    gearOne = int.Parse(match.Value);
                    foundGearOneOnRow = i;
                }
                else if (gearTwo == 0)
                {
                    gearTwo = int.Parse(match.Value);
                    foundGearTwoOnRow = i;
                }
                if (gearOne != 0 && gearTwo != 0 && foundGearOneOnRow == foundGearTwoOnRow)
                {
                    gear += gearOne * gearTwo;
                    gearOne = 0;
                    gearTwo = 0;
                }
                ...
                if (gearOne != 0 && gearTwo != 0 && foundGearOneOnRow != foundGearTwoOnRow)
                {
                    foundGearOneOnRow = foundGearTwoOnRow;
                    gearOne = gearTwo;
                    gearTwo = 0;
                }
                continue;
            ...
            if (i == 0)
            {
                for (var j = from; j <= to; j++)
                {
                    if (symbols2.Contains(rows[i + 1][j].ToString()))
                    {
                        if (gearOne == 0)
                        {
                            gearOne = int.Parse(match.Value);
                            foundGearOneOnRow = i+1;
                            gearOneIndex = j;
                        }
                        else if (gearTwo == 0)
                        {
                            gearTwo = int.Parse(match.Value);
                            foundGearTwoOnRow = i+1;
                            gearTwoIndex = j;
                        }
                ...
                for (var j = from; j <= to; j++)
                {
                    ...
                    if (symbols2.Contains(rows[i + 1][j].ToString()))
                    {
                        if (gearOne == 0)
                        {
                            gearOne = int.Parse(match.Value);
                            foundGearOneOnRow = i+1;
                            gearOneIndex = rows[i+1][endIndex-1];
                        }
                        else if (gearTwo == 0)
                        {
                            gearTwo = int.Parse(match.Value);
        ...
        if (gearOne != 0 && gearTwo != 0 && foundGearOneOnRow == foundGearTwoOnRow && gearOneIndex != gearTwoIndex)
        {
            gearTwoIndex = 0;
        }
